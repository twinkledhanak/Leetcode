...
// 2**d => d is the no of levels - not the level itself
// total nodes in last level: 2^d - 1
...
class Solution {
   ...
   public int countNodes(TreeNode root) {
    ...
    while (left <= right) {
      ...
      if (exists(pivot, d, root)) // Binary search would have: target < nums[mid] or target >= nums[mid]
        left = pivot + 1;
      else 
        right = pivot - 1;
    }
